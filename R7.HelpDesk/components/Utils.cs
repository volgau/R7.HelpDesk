...
namespace R7.HelpDesk
{
    public class Utils
    {
		...
		public static string StringLeft (string s, int length)
		{
		...
		public static bool IsFileAllowed (string filename)
		{
			...
			var allowedFileExts = hostSettings ["FileExtensions"].Split (new char[] { ',', ';' }, StringSplitOptions.RemoveEmptyEntries);

			var upExt = System.IO.Path.GetExtension (filename).ToLower ().TrimStart ('.');
			foreach (var ext in allowedFileExts)
				if (ext.Trim().ToLower () == upExt)
					return true;
		...
		private static IDictionary<string, string> _mimes = new Dictionary<string, string>(StringComparer.InvariantCultureIgnoreCase) 
		{
		...
		public static string GetMimeType(string extension)
		{
			...
			string mime;
			return _mimes.TryGetValue(extension, out mime) ? mime : "application/octet-stream";
        }
    }
}
